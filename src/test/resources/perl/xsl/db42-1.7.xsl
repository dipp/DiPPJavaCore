...
<xsl:template name="determineParGrouper">
  <xsl:param name="role" />
  <xsl:choose>
    <xsl:when test="string-length( $role ) = 1"><xsl:text></xsl:text></xsl:when> <!-- one space -->
    ...
    <xsl:otherwise>
      ...
      <xsl:variable name="groupBy">
        <xsl:call-template name="determineParGrouper">
          <xsl:with-param name="role"><xsl:value-of select="@class" /><xsl:text> </xsl:text></xsl:with-param>
        </xsl:call-template>
      </xsl:variable>
...
<!-- JS: this is for italic and bold style in a para -->
     <xsl:choose>
      <xsl:when test="@font-style='italic' and @font-weight='bold' and $inlines.graphical = 'true'">
        <emphasis role="bolditalic">
          <xsl:if test="@xml:lang">
...
<xsl:template match="linebreak">
  ...
  <!-- <xsl:text>&#x2028;</xsl:text> --> <!-- Unicode explicit line break character - should we use it? -->
<!--  <xsl:text>&#xa;</xsl:text>--> <!-- for now, write simple newline character -->
</xsl:template>
