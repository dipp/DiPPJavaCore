...
-->
    ...
    <!-- INLINES -->
    <xsl:param name="tagclasses">'personname' 'abbrev' 'acronym' 'emphasis' 'quote' 'trademark'
        'citation' 'citetitle' 'firstterm' 'glossterm' 'foreignphrase' 'wordasword' 'computeroutput'
        'literal' 'markup' 'prompt' 'replaceable' 'sgmltag' 'userinput' 'subscript' 'superscript'
        'accel' 'guibutton' 'guiicon' 'guilabel' 'guimenu' 'guimenuitem' 'guisubmenu' 'keycap'
        'keycode' 'keysym' 'mousebutton' 'action' 'classname' 'constant' 'errorcode' 'errorname'
        'errortype function' 'interface' 'msgtext' 'parameter' 'property' 'returnvalue'
        'structfield' 'structname' 'symbol' 'token' 'type' 'varname' 'application' 'command' 'envar'
        'filename' 'medialabel' 'option' 'systemitem' 'database' 'email' 'hardware' 'medialabel' 'optional'</xsl:param>
    <!-- Handling of Generated Text -->
    <xsl:param name="usePIsForGentext">false</xsl:param>
    <!-- Other options -->
    <xsl:param name="omitDefaultRole">true</xsl:param>
    <!-- if true, supresses writing role attributes when it is 'Normal' on <para>graphs and 'Default Paragraph Font' on <inline>s -->
    <!-- internal parameters -->
    <xsl:param name="excludeInAnnotation">table list par</xsl:param>
    <xsl:param name="ucBlockElements">document part section heading par list item footnote endnote
        table row cell entry tgroup index toc annotation</xsl:param>
    <xsl:param name="subdocelements">'header' 'footer' 'footnote' 'endnote' 'annotation' 'subdoc' 'textbox'</xsl:param>
    <!-- Output method: XML -->
    <xsl:output method="xml" indent="no" encoding="UTF-8"
        doctype-public="-//OASIS//DTD DocBook XML V4.2//EN"
        doctype-system="http://www.oasis-open.org/docbook/xml/4.2/docbookx.dtd" omit-xml-declaration="no"/>
    <!-- ####################
  DOCUMENT element
     ####################-->
    ...
    <xsl:template match="property[@name != 'title']">
        <xsl:choose>
            ...
            <xsl:when test="@name='editingMinutes'">
                <xsl:comment>
                    <xsl:text>Editing minutes: </xsl:text>
                    <xsl:value-of select="@value"/>
                </xsl:comment>
            </xsl:when>
            <!-- numberOfPages -->
            <xsl:when test="@name='numberOfPages'">
                <xsl:comment>
                    <xsl:text>Original number of pages: </xsl:text>
                    <xsl:value-of select="@value"/>
                </xsl:comment>
            </xsl:when>
            <!-- numberOfWords -->
            <xsl:when test="@name='numberOfWords'">
                <xsl:comment>
                    <xsl:text>Original number of words: </xsl:text>
                    <xsl:value-of select="@value"/>
                </xsl:comment>
            </xsl:when>
            <!-- numberOfChars -->
            <xsl:when test="@name='numberOfChars'">
                <xsl:comment>
                    <xsl:text>Original number of characters: </xsl:text>
                    <xsl:value-of select="@value"/>
                </xsl:comment>
            </xsl:when>
            <!-- numberOfCharsWS -->
            <xsl:when test="@name='numberOfCharsWS'">
                <xsl:comment>
                    <xsl:text>Original number of characters (incl. whitespace): </xsl:text>
                    <xsl:value-of select="@value"/>
                </xsl:comment>
            </xsl:when>
            <!-- revisionTime -->
            ...
            <xsl:when test="@name='lastPrintTime'">
                <printhistory>
                    <para>
                        <xsl:text>Last printed: </xsl:text>
                        <xsl:value-of select="@value"/>
                    </para>
                </printhistory>
            </xsl:when>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="count(./section) = count(./*)">
                        ...
                        <xsl:if test="$writeTitle = 'true'">
                            <title>
                                <xsl:value-of select="section[1]/heading"/>
                            </title>
                        </xsl:if>
                        <xsl:apply-templates/>
                    </xsl:when>
  ...
  PAR element
    ...
    <xsl:template match="par">
        ...
        <xsl:choose>
            <xsl:when test="($doPars = 'false') and contains($caption, @class) and                      ( count( preceding-sibling::*[position()=1 and name()='par' and count(descendant::image)&gt;0] )=1                       or count( preceding-sibling::*[position()=1 and name()='table'] )=1                       or                       count( following-sibling::*[position()=1 and name()='par' and count(descendant::image)&gt;0] )=1                       or count( following-sibling::*[position()=1 and name()='table'] )=1                     )">
                <!--
...
-->
           ...
           Therefore, we need to determine some things first, which we store in a variable. -->
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$groupBy != ''">
                        <xsl:choose>
                            <xsl:when test="$groupBy='literallayout'">
                                <!-- need to strip par's -->
                                <xsl:choose>
        ...
        <xsl:if test="count(.//gentext[@type='INDEX']) &gt; 0">
            <index>
                <xsl:comment> Index is to be generated here by processing system </xsl:comment>
            </index>
        </xsl:if>
    </xsl:template>
  ...
  INLINE element
    ...
    <xsl:template match="inline">
        <xsl:variable name="isElem">
            <xsl:value-of select="contains( $tagclasses, concat( &quot;&apos;&quot;, @class, &quot;&apos;&quot;) ) and @class!=''"/>
        </xsl:variable>
        <xsl:choose>
            ...
            <xsl:when test="(@vertical-align!='baseline' and @vertical-align!='sub') and (not( starts-with( @vertical-align, '-' )) or @vertical-align='super')">
                <superscript>
                    ...
                    <xsl:apply-templates select="child::node()[name() != 'footnote' and name() != 'endnote' and name() != 'image']"/>
                    <!-- no footnotes, endnotes, images in superscript elements! -->
                </superscript>
            ...
            <!-- emphasis -->
            <!-- for now, don't do the following by default.
    ...
    <xsl:template match="list">
        ...
        <xsl:choose>
            <xsl:when test="($lt='disc') or ($lt='square') or ($lt='circle') or ($lt='hyphen')">
                <itemizedlist mark="{$lt}">
                    <xsl:attribute name="spacing">
                        <xsl:choose>
                            <xsl:when test="starts-with(@ilx-marker-offset, '-')">
                                <!-- negative indent -> normal list! -->
            ...
            <xsl:when test="($lt='lower-roman')">
                <orderedlist numeration="lowerroman">
                    ...
                    <xsl:if test="string-length( substring-after( substring-after(@ilx-marker-format, '%'), '%')) &gt; 0">
                        <!-- at least two 2 '%'s -->
...
-->
                    ...
                    <xsl:when test="count(ancestor::par[1]/following-sibling::*[position()=1 and name()='par' and contains($caption, @class)]) &gt; 0                         and count(ancestor::par[1]/descendant::image) = 1">
                    ...
                    <xsl:when test="count(ancestor::par[1]/preceding-sibling::*[position()=1 and name()='par' and contains($caption, @class)]) &gt; 0                         and count(ancestor::par[1]/descendant::image) = 1">
                    ...
                    <imageobject>
                        <xsl:element name="imagedata">
                            ...
                            <xsl:attribute name="format">
                                <xsl:value-of select="my-class:determineImageFormat(@xlink:href)"/>
                            </xsl:attribute>
    ...
    <!--
<xsl:template match="par[]//image"> <!- - when image is only child in paragraph - ->
  <mediaobject>
    <imageobject>
      <xsl:element name="imagedata">
        <xsl:choose>
    ...
    <caption>
      <xsl:choose>
        <!- - First, check for caption AFTER the image - ->
        <xsl:when test="count(ancestor::par/following-sibling::*[position()=1 and name()='par' and contains($caption, @class)]) > 0
        ...
        <!- - ...and only then before. - ->
        <xsl:when test="count(ancestor::par/preceding-sibling::*[position()=1 and name()='par' and contains($caption, @class)]) > 0
                        and count(ancestor::par[1]/descendant::image) = 1">
          <xsl:apply-templates select="ancestor::par[1]/preceding-sibling::*[position()=1 and name()='par' and contains($caption, @class)]">
            <xsl:with-param name="doPars">true</xsl:with-param>
          </xsl:apply-templates>
  ...
  REFERENCE element
    ...
    <xsl:template match="reference">
        <xsl:choose>
            ...
            <xsl:when test="count(//footnote[my-class:makeValidID(@rawtarget)=my-class:makeValidID(current()/@rawtarget)]) &gt; 0                     or                     count(//endnote[my-class:makeValidID(@rawtarget)=my-class:makeValidID(current()/@rawtarget)]) &gt; 0">
                <footnoteref>
                    <xsl:attribute name="linkend">
                        <xsl:value-of select="my-class:makeValidID(@rawtarget)"/>
                    </xsl:attribute>
                </footnoteref>
            </xsl:when>
            <xsl:otherwise>
...
-->
    ...
    <xsl:template match="gentext">
        <xsl:choose>
            ...
            <xsl:when test="(@type = 'INDEX')">
                <xsl:if test="count(ancestor::par)=0">
                    <!-- not in a par element -->
                    <index>
                        <xsl:comment> Index is to be generated here by processing system </xsl:comment>
                    </index>
                </xsl:if>
  ...
  INDEXTARGET element
    ...
    <xsl:template name="indextarget" match="indextarget">
        <indexterm>
            ...
            <xsl:if test="altref">
                <see>
                    <xsl:apply-templates select="altref"/>
                </see>
            </xsl:if>
        </indexterm>
        <xsl:if test="(@entryLevel4!='') or (@entryLevel4!='') or (@entryLevel4!='') or (@entryLevel4!='') or (@entryLevel4!='') or (@entryLevel4!='')">
            <xsl:message>index term '<xsl:value-of select="@entryLevel1"/>' has more than the 3
                levels DocBook allows</xsl:message>
        </xsl:if>
    </xsl:template>
  ...
  TABLE element
    ...
    <xsl:template match="table">
        ...
        <xsl:variable name="tt">
            <xsl:choose>
                <xsl:when test="count(ancestor::footnote) &gt; 0                        or count(ancestor::endnote) &gt; 0                       or not( preceding-sibling::*[position()=1 and name()='par' and contains($caption, @class)]                               or                               following-sibling::*[position()=1 and name()='par' and contains($caption, @class)] )                      ">
            ...
            <xsl:if test="$tt = 'table'">
                <xsl:choose>
                    <xsl:when test="following-sibling::*[position()=1 and name()='par' and contains($caption, @class)]">
                        <title>
                            <xsl:value-of select="following-sibling::par[1]"/>
                        </title>
                    </xsl:when>
                    <xsl:when test="preceding-sibling::*[position()=1 and name()='par' and contains($caption, @class)]">
                        <title>
                            <xsl:value-of select="preceding-sibling::par[1]"/>
                        </title>
                    </xsl:when>
                    <xsl:otherwise>
  ...
  nested TABLE element
    ...
    <xsl:template match="entry/table">
        ...
        <xsl:comment> ### complete table dropped: nested tables not supported in CALS (use HTML
            table model!) ### </xsl:comment>
    </xsl:template>
    <!-- ####################
  COLUMNBREAK element
     ####################-->
